...
export module external.zstring_view;

import std;

export
namespace external::mpt
{
    ...
    public:
        ...
        constexpr basic_zstring_view(const std::basic_string<CharT, Traits>& s) noexcept : m_view{s} {}
        ...
        requires requires (T v) {
            ...
            { v.c_str() } -> std::same_as<const_pointer>;
        }
        constexpr basic_zstring_view(T&& v) : m_view{v.c_str(), v.size()} {}
        ...
        {
            return basic_zstring_view{_private_tag(), m_view.substr(start, npos)};
        }
        ...
        friend constexpr auto operator<=>(basic_zstring_view, basic_zstring_view) = default;

    private:
        enum struct _private_tag {};
        ...
        explicit constexpr basic_zstring_view(_private_tag, const underlying_type& v) noexcept : m_view{v} {}
...
export
template<typename CharT, typename Traits>
struct std::formatter<external::mpt::basic_zstring_view<CharT, Traits>>
    : std::formatter<typename external::mpt::basic_zstring_view<CharT, Traits>::underlying_type> 
{};
